...
The Line class should have the type property equal to 'line' and the points property, which is an array of Point class objects (points that form a line). 
The constructor of the constructor takes an array of point coordinates in the format [[x1, y1], [x2, y2], ...] (e.g. [[0, 0], [10, 20], [20, 20]]).
...
fromJSON - takes JSON data, parses it, and adds to the collection. 
As an additional argument, provide a flag that specifies whether the data should be added to an existing collection or replace it;
...
class Figure{
    ...
    fromJSON(json,flag=false){
        let jObject = JSON.parse(json);
        if(flag){
            this.elements.points = this.elements.points.concat(jObject.points||[]);
            this.elements.lines = this.elements.lines.concat(jObject.lines||[]);
        } else{
            this.elements = jObject;
        }
    }
