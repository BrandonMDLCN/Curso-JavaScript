...
Create a User class to create objects for both teachers and students. 
The constructor should take the user data (name, surname, email, role), but be sure to create the appropriate properties.

Additionally, create the following methods:

addCourse(course, level) - which will allow you to add course (e.g. math) and level (e.g. 2 - the higher the number, the higher the level); 
in the case of a student, it will mean that they are looking for help on this level, and in case of a teacher, it will mean that they can help up to this level;
...
complete information about the sent message should be stored in the local cache (hint: use an array for this); 
the sending of the message itself will only be simulated, declare the function sendEmail(from, to, message) {} beforehand and use it in the appropriate place;
...
let student11 = new User({name: 'Rafael', surname: 'Fife', email: 'rfife@rhyta.com', role: 'student'});
let student22 = new User({name: 'Kelly', surname: 'Estes', email: 'k_estes@dayrep.com', role: 'student'});
let teacher11 = new User({name: 'Paula', surname: 'Thompkins', email: 'PaulaThompkins@jourrapide.com', role: 'teacher'});

student11.addCourse('maths', 2);
student11.addCourse('physics', 1);
student11.removeCourse('physics');
teacher11.addCourse('biology', 3);
teacher11.editCourse('biology', 4);
console.log(`${student11.name}: ${student11.courses.length} courses`); // -> Rafael: 1 courses
console.log(`${teacher11.name}: ${teacher11.courses.length} courses`); // -> Paula: 1 courses
teacher11.sendMessage(student11, 'test message');
teacher11.sendMessage(student11, 'another message');
teacher11.showMessagesHistory();
// -> rfife@rhyta.com -> PaulaThompkins@jourrapide.com: test message
// -> rfife@rhyta.com -> PaulaThompkins@jourrapide.com: another message